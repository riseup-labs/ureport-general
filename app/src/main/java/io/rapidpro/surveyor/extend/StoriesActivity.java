...
//        }
        ...
        if(storiesLocal.getTitle_bn() != null){ bangla_title = storiesLocal.getTitle_bn(); }
        if(storiesLocal.getSubtitle_bn() != null){ bangla_body = storiesLocal.getSubtitle_bn(); }
        if(storiesLocal.getBody_bn() != null){ bangla_body = bangla_body + "<br>" + storiesLocal.getBody_bn().replace("\r\n", "<br>"); }

        String english_title = "";
        String english_body = "";

        if(storiesLocal.getTitle_en() != null){ english_title = storiesLocal.getTitle_en(); }
        if(storiesLocal.getSubtitle_en() != null){ english_body = storiesLocal.getSubtitle_en(); }
        if(storiesLocal.getBody_en() != null){ english_body = english_body + "<br>" + storiesLocal.getBody_en().replace("\r\n", "<br>"); }

        WebContent = WebContent.replace("#Title_Lang_1", english_title);
        WebContent = WebContent.replace("#Title_Lang_2", bangla_title);
        WebContent = WebContent.replace("#Body_Lang_1", english_body);
        WebContent = WebContent.replace("#Body_Lang_2", bangla_body);
        WebContent = WebContent.replace("::lang_2_key::", "Burmese");
        WebContent = WebContent.replace("::lang_2_text::", "বাংলা");
