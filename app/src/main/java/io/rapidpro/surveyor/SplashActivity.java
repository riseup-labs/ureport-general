    ...
    void requestPermission() {
        Permiso.getInstance().requestPermissions(new Permiso.IOnPermissionResult() {
            ...
            public void onRationaleRequested(Permiso.IOnRationaleProvided callback, String... permissions) {
                SplashActivity.this.showRationaleDialog(R.string.permission_storage, callback);
            }
                ...
                public void onResponse(Call<story_api> call, Response<story_api> response) {
                    ...
                    new Thread(() -> {
                        ...
                        for (StoriesLocal s : localStories) {
                            ...
                            if (file.exists()) {
                                if (file.length() >= s.getContent_image_size() - (10*1024)) {
                                    // File with At Least 10 KB Data Exists: skip
                                    continue;
                                }
                            }
                            ...
                            getURL(imageURL, new okhttp3.Callback() {
                                ...
                                public void onResponse(okhttp3.Call call, okhttp3.Response response) throws IOException {
                                    ...
                                    String file_path_x = "story_image_" + getMD5(imageURL);
                                    try (OutputStream output = context.openFileOutput(file_path_x, context.MODE_PRIVATE)) {
                        ...
                        for (StoriesLocal s : localStories) {
                            ...
                            File file = new File(file_path_full);
                            if (file.exists()) {
                                if (file.length() >= s.getStory_video_size() - (10*1024)) {
                                    // File with At Least 10 KB Data Exists: skip
                                    continue;
                                }
                            }
                            ...
                            getURL(videoURL, new okhttp3.Callback() {
                                ...
                                public void onResponse(okhttp3.Call call, okhttp3.Response response) throws IOException {
                                    InputStream inputStream = response.body().byteStream();
                                    String file_path_x = "story_video_" + getMD5(videoURL);
                                    try (OutputStream output = context.openFileOutput(file_path_x, context.MODE_PRIVATE)) {
